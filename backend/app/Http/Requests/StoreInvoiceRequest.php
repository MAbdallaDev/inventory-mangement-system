...
class StoreInvoiceRequest extends FormRequest
{
    ...
    {
        return [
            'customer_info.id' => ['nullable', 'max:255', 'exists:clients,id'],
            'customer_info.name' => ['nullable', 'string', 'min:1', 'max:250'],
            'customer_info.phone' => ['nullable', 'min:1', 'max:25'],
            'customer_info.address' => ['nullable', 'string', 'min:1', 'max:250'],

            'invoice_info.invoiceId' => ['required', 'min:1', 'max:250'],
            'invoice_info.invoiceDate' => ['required', 'min:1', 'max:250'],
            'invoice_info.dueDate' => ['required', 'min:1', 'max:250'],
            'invoice_info.paymentMethod' => ['required', 'string', 'min:1', 'max:250'],

            'order_details.description' => ['required', 'string', 'min:1', 'max:250'],
            'order_details.discount' => ['required', 'max:250'],
            'order_details.totalQuantity' => ['required', 'max:250'],
            'order_details.totalPrice' => ['required', 'max:250'],


            'order_details.items.*.name' => ['required', 'string', 'min:1', 'max:250', 'exists:products,name'],
            'order_details.items.*.color' => ['required', 'min:1', 'max:250', 'exists:colors,id'],
            'order_details.items.*.size' => ['required', 'min:1', 'max:250', 'exists:sizes,id'],
            'order_details.items.*.quantity' => ['required', 'min:1', 'max:250'],
            'order_details.items.*.price' => ['required', 'min:1', 'max:250'],
